...
### Option 1: Docker (One command to run everything)
...
curl -X POST http://localhost:8000/api/v1/chat \
  ...
  -d '{"message": "Hi! My name is Gabi."}'
```

Response:
```json
{
  "response": "Hello Gabi! How can I help you today?",
...
```json
{
  "response": "Your name is Gabi, as you mentioned earlier.",
  "session_id": "a1b2c3d4-e5f6-7890-abcd-ef1234567890"
}
```
...
To reduce hallucinations in RAG systems, several complementary techniques can be implemented in the prompt. Establishing a context boundary with instructions like "answer based ONLY on the provided context" prevents the model from injecting knowledge from its training. Granting explicit permission to refuse queries through phrases like "if you don't have the information, say 'I don't have information about that'" allows the model to admit its limitations instead of fabricating responses. Requiring citations with "cite document names when possible" forces the model to ground its answers in the provided sources. Finally, requesting that it acknowledge uncertainty with "if you're unsure, acknowledge the uncertainty" reduces hallucinations generated by overconfidence.  
...
| **Custom Memory** | Simple dict-based implementation |
...
For the development of this project, **GitHub Copilot** was used primarly for planning, API architecture design, Mermaid diagrams, documentation formalization, and directory structure design. Its output was always cross-checked against up-to-date official documentation and guided through strict, specific, and well-defined prompts.

### Limitations and corrections: example case

During the exploration of the best approach to provide conversational memory to the LLM-based chat, the coding assistant was asked to suggest options for integrating memory using existing LangChain tools. As a result, it suggested using `ConversationBufferMemory`, which is currently deprecated and present in the `langchain-classic` package.

After reviewing the official documentation, it became clear that LangChain now recommends using `LangGraph` checkpointers for managing conversational state. Given this deprecation and the added complexity of `LangGraph` for the scope of this project, the suggested approach was revised.

Instead, a simpler and more explicit solution was implemented: conversational memory is maintained using a `session_id` generated on the frontend, sent to the API, and reused throughout the entire conversation. When no `session_id` is provided, the system assumes that a new conversation has started. This decision reduced unnecessary complexity while maintaining correct conversational context handling.
